...
let watchlistStatus = JSON.parse(localStorage.getItem("watchlistStatus")) || {};
...
localStorage.setItem("watchlistStatus", JSON.stringify(watchlistStatus));
   ...
   function deleteFromWatchlist(){
    axios
      .delete("http://localhost:5000/watchlistdelete",{data:{space_id}})
      .then((res)=>{
  ...
  return (
    <>
      ...
      <div className="">
        <button className="ml-3 mt-3 px-5 py-1 border border-gray-300 font-semibold text-xl cursor-pointer bg-stone-100 hover:bg-stone-200 transition duration-400" onClick={backBtn}>Back</button>
        <div className="mt-5 mb-5 w-[90%] mx-auto justify-center">
          ...
          <div>
            <p className="font-semibold text-xl mb-2">Indetail Description:</p>
          ...
          <div className="mt-3">
          <p className="font-semibold italic ml-2 mb-1 text-xl text-gray-600">{watchlistFeedback}</p>
        <button className="px-3 py-2 rounded-lg border-gray-500 bg-gray-300 font-bold text-xl cursor-pointer hover:bg-gray-400 hover:border border-black-500 active:scale-93 transition duration-500" onClick={deleteFromWatchlist}>Delete From Watchlist</button>
          </div>
